...
function addGamesToPage(games) {
    ...
    // Step 1: loop over each game
  for (let i = 0; i < games.length; i++) {
    let game = games[i];  // current game object

    // Step 2: create a new div for the game card
    let gameCard = document.createElement("div");
    gameCard.classList.add("game-card");

    // Step 3: set inner HTML with game info
    gameCard.innerHTML = `
    ...
    // Append the game card to the container on the page
...
const displayStr = `
  A total of $${totalRaised.toLocaleString("en-US")} has been raised for ${totalGames} game${totalGames === 1 ? "" : "s"}.
